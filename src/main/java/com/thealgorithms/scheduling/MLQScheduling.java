...
class Process1 {
  ...
  public Process1(String name, int priority, int burstTime) {
    this.name = name;
...
public class MLQScheduling {
  ...
  ArrayList<Queue<Process1>> queues;
  int numQueues;
  ...
  public void addProcess(Process1 process) {
  ...
  public void run() {
    while (!queues.get(0).isEmpty() || !queues.get(numQueues - 1).isEmpty()) {
      ...
      for (int i = 0; i < numQueues; i++) {
        Queue<Process1> currentQueue = queues.get(i);
        if (!currentQueue.isEmpty()) {
          // Get the next process from the current queue
          Process1 currentProcess = currentQueue.poll();
  ...
  public static void main(String[] args) {
    ...
    scheduler.addProcess(new Process1("P1", 0, 5));
    scheduler.addProcess(new Process1("P2", 1, 4));
    scheduler.addProcess(new Process1("P3", 2, 3));
    scheduler.addProcess(new Process1("P4", 0, 2));
