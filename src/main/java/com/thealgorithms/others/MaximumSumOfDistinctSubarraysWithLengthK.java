...
public class MaximumSumOfDistinctSubarraysWithLengthK {
    /*
     * Returns the maximum sum of subarray of size K consisting of distinct elements.
     * 
     * @param k size of the subarray which should be considered from the given array.
     * @param nums is the array from which we would be finding the required subarray.
     *
    ...
    public static long maximumSubarraySum(int k,int ...nums) {
        ...
        long s = 0; // this will store the sum of every k elements which can be used to compare with max
        ...
        //Checking if the first kth subarray contains unique elements or not if so then we assign that to max
        if (set.size() == k) {
            max = s;
        }
//Looping through the rest of the array to find different subarrays and also utilising the sliding window algorithm to find the sum
//in O(n) time complexity
        for (int i = 1; i < nums.length - k + 1; i++) {
            ...
            set.add(nums[i + k - 1]);
            //if the subarray contains distinct elements then we compare and update the max value
            if (set.size() == k) {
                if (max < s) {
                    max = s;
                }
