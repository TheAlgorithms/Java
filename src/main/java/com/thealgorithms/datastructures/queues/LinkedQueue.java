...
public class LinkedQueue<T> implements Iterable<T> {
	...
	/**
	 * Peeks the element at the index and returns the value
	 * 
	...
	public T peek(int pos) {
		if (pos > size)
			throw new IndexOutOfBoundsException("Position %s out of range!");
		Node<T> node = front;
		while (pos-- > 0)
			node = node.next;
		return node.data;
	...
	/**
	 * Node iterator, allows to travel through the nodes using for() loop or
	 * forEach(Consumer)
	 */
