 ...
 * 
 * Given the head of a linked list, you need to reverse the nodes of the list k
 * at a time and return the modified list.
 * k is a positive integer and is less than or equal to the length of the linked
 * list.
 * If the number of nodes is not a multiple of k, then the left-out nodes at the
 * end should remain as they are.
 ...
 * you can solve the problem without using extra memory (i.e., without creating
 * a new list) while still reversing the nodes in groups of k. Solving it in
 * O(1) extra memory space typically requires manipulating the pointers within
...
public class ReverseNodesKGroup {
  ...
  public ListNode reverseKGroup(ListNode head, int k) {
    ...
    while (pointer != null) { // loop while the node referenced by pointer is not null
      ...
      if (temp == null)
        break; // if the node referenced by temp is null, break (i.e. not enough nodes for a
               // full k-group)
      ...
      ListNode current = pointer.next; // pointer to reference the current node (initially set to head of the current
                                       // k-group)
