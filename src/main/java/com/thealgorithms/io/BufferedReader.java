...
/**
 * Mimics the actions of the Original buffered reader implements other actions,
 * such as peek(n) to lookahead, block() to read a chunk of size {BUFFER SIZE}
 * <p>
...
public class BufferedReader {
	...
	/**
	 * Maximum number of bytes the buffer can hold. Value is changed when
	 * encountered Eof to not cause overflow read of 0 bytes
	 */
	...
	public int peek(int n) throws IOException {
		...
		if (n >= available)
		{
			String msg = "Out of range, available %d, but trying with %d";
	...
	/**
	 * Removes the already read bytes from the buffer in-order to make space for new
	 * bytes to be filled up.
	 * <p>
	...
	/**
	 * Reads one complete block of size {bufferSize} if found eof, the total length
	 * of array will be that of what's available
	 *
