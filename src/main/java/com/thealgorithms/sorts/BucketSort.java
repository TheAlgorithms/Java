...
/**
 * BucketSort class provides a method to sort an array of elements using the
 * Bucket Sort algorithm and implements the SortAlgorithm interface.
 */
public class BucketSort implements SortAlgorithm {
	 ...
	 */
	private <T extends Comparable<T>> void distributeElementsIntoBuckets(T[] array, List<List<T>> buckets, final T min,
			final T max, final int numberOfBuckets) {
	...
	/**
	 * The method computes the index of the bucket in which a given element should
	 * be placed. This is done by "normalizing" the element within the range of the
	 * array's minimum (min) and maximum (max) values, and then mapping this
	 * normalized value to a specific bucket index.
	 *
