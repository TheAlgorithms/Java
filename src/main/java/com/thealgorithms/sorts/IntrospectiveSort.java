 ...
 *
 * @see <a href="https://en.wikipedia.org/wiki/Introsort">IntroSort
 *      Algorithm</a>
 */
public class IntrospectiveSort implements SortAlgorithm {
	...
	/**
	 * Sorts the given array using Introspective Sort, which combines quicksort,
	 ...
	 * @param <T>   The type of elements in the array, which must be comparable
	 */
	private static <T extends Comparable<T>> void introspectiveSort(T[] array, final int low, int high,
			final int depth) {
	...
	private static <T extends Comparable<T>> void heapify(T[] array, final int i, final int n, final int low) {
		...
		if (right < n && SortUtils.greater(array[low + left], array[low + largest])) {
			largest = right;
		}
		if (largest != i) {
