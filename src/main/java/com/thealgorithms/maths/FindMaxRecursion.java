...
public class FindMaxRecursion {
     ...
     * @return max of {@code array}
     */
    public static double max(double[] array, int low, int high) {
        if (low == high) {
            return array[low]; // or array[high]
        }
        ...
        double leftMax = max(array, low, mid); // get max in [low, mid]
        double rightMax = max(array, mid + 1, high); // get max in [mid+1, high]
