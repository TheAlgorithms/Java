 ...
 * Output :
 * 
 * Enter number of distinct letters 6 Enter letters with its frequncy to encode
 * Enter letter : a Enter frequncy : 45
 * 
 * Enter letter : b Enter frequncy : 13
 * 
 * Enter letter : c Enter frequncy : 12
 * 
 * Enter letter : d Enter frequncy : 16
 * 
 * Enter letter : e Enter frequncy : 9
 * 
 * Enter letter : f Enter frequncy : 5
 * 
 * Letter Encoded Form a 0 b 1 0 1 c 1 0 0 d 1 1 1 e 1 1 0 1 f 1 1 0 0
 * 
 */
...
public class Huffman {
	...
	public static Node make_huffmann_tree(List li) {
		...
		while (true) {
			...
			// here first assignment of it.next in a will always be true as list
			// till end will
			// must have atleast one node
			a = (Node) it.next();
			// Below condition is to check either list has 2nd node or not to
			// combine
			// If this condition will be false, then it means construction of
			// huffman tree is completed
			if (it.hasNext()) {
				b = (Node) it.next();
			}
			// Combining first two smallest nodes in list to make its parent
			// whose frequncy
			...
			if (b != null) {
				...
				temp.left = a;
				temp.right = b;
				// after combing, removing first two nodes in list which are
				// already combined
				li.remove(0);// removes first element which is now combined
								// -step1
				li.remove(0);// removes 2nd element which comes on 1st position
								// after deleting first in step1
				li.add(temp);// adding new combined node to list
				// print_list(li); //For visualizing each combination step
			}
			// Sorting after combining to again repeat above on sorted frequency
			// list
			li.sort(new comp());
			it = li.iterator();// resetting list pointer to first node
								// (head/root of tree)
			if (li.size() == 1) {
				return (Node) it.next();
	...
	public static void dfs(Node n, String ch) {
		...
		int freq = n.freq; // recording root freq to avoid it adding in path
							// encoding
	...
	public static void find_path_and_encode(Stack<Node> st, Node root, String s, int f) {
		...
		if (root != null) {
			...
			// Popping if path not found in right or left of this node,because
			// we previously
